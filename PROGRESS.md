...
>
> 최종 갱신: **2026-07-21** · 진행: **백엔드 T-14까지 + 프론트 T-F1 완료. 다음은 프론트 T-F4(대시보드) 또는 백엔드 T-15**
...
- [ ] **T-F2** 온보딩 위저드
- [ ] **T-F3** 리뷰 인박스 + 답글 드로어
- [ ] **T-F4** 대시보드 화면 (Recharts) ← **다음 후보 (화면이 살아나는 곳)**
- [ ] **T-F5** 리포트·비교 화면
- [ ] **T-F6** PWA + 웹푸시
- [ ] **T-F7** AI 비서 채팅(SSE)
...
- **백엔드 pytest 84개 전부 통과**, `ruff check` 통과 (모든 태스크 공통 완료조건).
- **프론트 `npm run build` 통과** (10개 라우트, 미들웨어). `npm run gen:api`로 백엔드 OpenAPI→TS 타입 생성 확인.
