...
POS_TEMPLATES = [
    ...
    (
        "단골 확정입니다. 맛도 서비스도 다 만족스러워요.",
        [("맛", "pos"), ("친절", "pos")],
        [],
        (5, 5),
    ),
    (
        "인테리어가 감성적이라 사진 찍기 좋아요. 커피도 맛있고요.",
        [("분위기", "pos"), ("맛", "pos")],
        [],
        (4, 5),
    ),
...
NEG_TEMPLATES = [
    ...
    (
        "주문이 누락돼서 30분을 더 기다렸습니다. 사과도 없네요.",
        [("친절", "neg"), ("대기시간", "neg")],
        ["주문누락", "불친절"],
        (1, 1),
    ),
...
REPLY_TEMPLATES = {
    ...
    "neg": (
        "불편을 드려 정말 죄송합니다. 말씀 주신 내용은 바로 개선하겠습니다. "
        "다시 기회를 주시면 좋은 모습으로 보답하겠습니다."
    ),
}
...
def _diag(level: str, title: str, evidence: str) -> dict:
    return {"level": level, "title": title, "evidence": evidence}


def _rx(title: str, detail: str, effect: str) -> dict:
    return {"title": title, "detail": detail, "expected_effect": effect}
    ...
    0: (
        [
            _diag(
                "crit",
                "주말 웨이팅 불만 급증",
                "최근 4주 '대기시간' 부정 언급 다수, '웨이팅' 키워드 상위",
            ),
            _diag("warn", "위생 관련 지적 반복", "'청결' 부정 언급이 매주 발생"),
            _diag("strength", "맛 만족도는 상승세", "'맛' 긍정 비중이 직전 기간보다 상승"),
            _diag(
                "opportunity",
                "분위기 호평 → SNS 홍보 기회",
                "'분위기' 긍정 언급과 사진 리뷰 비중 높음",
            ),
        ],
        [
            _rx(
                "주말 예약제/대기 알림 도입",
                "네이버 예약 또는 테이블링으로 대기 스트레스 완화",
                "대기시간 부정 리뷰 감소",
            ),
            _rx(
                "마감 청소 체크리스트 운영",
                "테이블·바닥·화장실 일일 점검표 작성",
                "청결 지적 재발 방지",
            ),
            _rx("포토존 해시태그 이벤트", "인스타 업로드 시 음료 사이즈업", "신규 방문 유입 증가"),
        ],
    ),
    1: (
        [
            _diag("crit", "가격 인상 후 불만 증가", "'가격' 부정 언급과 '가격인상' 키워드 급증"),
            _diag("warn", "긍정 비율 하락 추세", "긍정 비율이 직전 기간 대비 하락"),
            _diag("strength", "단골 중심의 맛 호평 유지", "'맛' 긍정 언급 비중 안정적"),
            _diag("opportunity", "점심 세트 수요 존재", "'가격' 언급 리뷰에서 세트/구성 요구 반복"),
        ],
        [
            _rx(
                "점심 한정 세트 메뉴 출시",
                "김치찜+공기밥+음료 구성으로 체감 가격 방어",
                "가격 불만 완화, 점심 회전율 상승",
            ),
            _rx(
                "가격 인상 사유 안내문 게시",
                "원재료 가격 변동 안내로 납득도 제고",
                "부정 리뷰 톤 완화",
            ),
            _rx("재방문 스탬프 쿠폰", "5회 방문 시 1인분 무료", "단골 락인 강화"),
        ],
    ),
}
