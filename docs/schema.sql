-- =============================================================================
-- 리뷰 진단 AI SaaS — DB 스키마 (원본. migrations/001_init.sql 은 이 파일에서 출발)
-- 기반: docs/TECH_SPEC.md §4.1 DDL 초안 + ARCHITECTURE.md §3-7 에서 추가한 테이블
-- 로컬 개발(docker postgres)에서는 auth.users 가 없으므로 dev 전용 스텁 사용(하단 참고)
-- =============================================================================
...
create table review_analysis (
  ...
  alerted_at    timestamptz,                   -- 알림 발송 시각(미발송=null). digest 중복 방지 (migration 003)
...
create table crawl_jobs (
  ...
  error         text,
  collected     int,                           -- 수집 건수 (migration 002). 자가진단·성공률 수치 원천
  created_at    timestamptz default now(),
...
-- Supabase 환경에서만 적용. 정책을 하나도 만들지 않으면 anon/authenticated는 전부 거부됨
-- =============================================================================
...
-- =============================================================================
-- [로컬 개발 전용 — Supabase에는 적용하지 말 것]
-- docker postgres에는 auth 스키마가 없으므로, dev 마이그레이션에서만 아래를 먼저 실행:
